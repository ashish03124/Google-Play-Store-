...
try:
    ...
    reviews_df = pd.read_csv('apps_reviews_40.csv')  # Assuming you have this file
except FileNotFoundError as e:
    print(f"Error loading files: {e}")
    # For demonstration, I'll create a minimal dataframe from the sample you provided
...
def clean_playstore_data(df):
    ...
    df['Installs'] = (
        ...
        .fillna(0)  # Fill any remaining NA with 0
    ...
    df['Size'] = (
        df['Size']
        .astype(str)
        .str.replace('M', 'e6')  # Convert M to millions
        .str.replace('k', 'e3')  # Convert k to thousands
        .str.replace('Varies with device', 'NaN')  # Handle special case
        .apply(pd.to_numeric, errors='coerce')  # Convert to numeric
    )
    
    # Clean 'Price' column - handle cases where it might contain non-numeric values
    df['Price'] = (
        df['Price']
        .astype(str)
        .str.replace('$', '')
        .replace('Everyone', '0')  # Replace 'Everyone' with '0'
        .apply(pd.to_numeric, errors='coerce')  # Convert to numeric
...
# Prepare and train ML model if we have enough data
if len(playstore_df) > 100:  # Arbitrary threshold for demonstration
